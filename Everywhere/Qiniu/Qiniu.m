...
-(instancetype)init{
    self=[super init];
    if (self) {
        self.QNURL=@"http://www.7xpt9o.com1.z0.glb.clouddn.com/";
...
+(NSURL *)downloadURLForFile:(NSString *)fileName urlString:(NSString *)urlString accessKey:(NSString *)accessKey secretKey:(NSString *)secretKey{
    ...
    NSString *sign=[GTM_Base64 stringByWebSafeEncodingData:signData padded:YES];
...
+(NSString *)accessToken:(NSString *)string accessKey:(NSString *)accessKey secretKey:(NSString *)secretKey{
    ...
    NSString *urlsafe_base64_encoded_signString=[GTM_Base64 stringByWebSafeEncodingData:signData padded:YES];
    //按规定格式生成AccessToken
...
//-(BOOL)deleteQNFile:(NSString *)name{
//    return [self deleteQNFile:name bucketName:self.QNBucketName accessKey:self.QNAccessKey secretKey:self.QNSecretKey];
//}
//
//-(BOOL)deleteQNFile:(NSString *)name bucketName:(NSString *)bucketName accessKey:(NSString *)accessKey secretKey:(NSString *)secretKey{
//    //1.生成requestURI
//    NSString *entry=[NSString stringWithFormat:@"%@:%@",bucketName,name];
...
+(NSURLRequest *)moveURLRequestFromFile:(NSString *)srcFileName toFile:(NSString *)destFileName bucketName:(NSString *)bucketName accessKey:(NSString *)accessKey secretKey:(NSString *)secretKey{
    ...
    NSString *encodedEntryURISrc=[GTM_Base64 stringByWebSafeEncodingData:entryDataSrc padded:YES];
